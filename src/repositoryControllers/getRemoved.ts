 ...
 * @returns
 */
export function getRemoved<UserType extends User, DocType extends Document>(
	modelDeclaration: ModelDeclaration<UserType, DocType>
) {
    return async (
		req: Request<UserType, RepoControllersArgumentsTypes<DocType>["getRemoved"]>,
		res: Response<RepoControllersReturnTypes<DocType>["getRemoved"]>
	) => {
		try {
			...
			// La requête getRemoved ne renvoie les document supprimés
			//@ts-ignore
			...
			modelDeclaration.model.find(queryFilter).lean<DocType>().exec(async (err: any, docs: DocType[]) => { // On recherche les document demandée par le queryFilter
				if(err) { // Si mongosse a envoyé une erreur, la requête a échoué, on renvoi un tableau vide
					log.error(`Error in ${modelDeclaration.name}/getRemoved.`, err); // Une erreur inconnue s'est produite.
				...
				// La requête a réussi. On a tous les documents en fonction du query filter. Il reste un autre tri à effectuer.
					...
					for(const doc of docs) { // Pour tous les documents que mongoogse a trouvé.
						try {
							...
							log.error(`Error in ${modelDeclaration.name}/getRemoved en appelant ${modelDeclaration.name}.permissions.request.`, e); // Une erreur s'est produite.
						}
					}
			...
			log.error(`Error in ${modelDeclaration.name}/getRemoved.`, e); // Une erreur s'est produite.
